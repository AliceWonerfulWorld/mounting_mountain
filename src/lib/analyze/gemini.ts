...
function getApiErrorDetails(error: unknown): { status?: number; message: string } {
    ...
    if (typeof error === "object" && error !== null) {
        ...
        const responseStatus = typeof maybeResponse === "object" && maybeResponse !== null && "status" in maybeResponse
            ? maybeResponse.status
            : undefined;
        const maybeMessage = "message" in error ? error.message : undefined;

        return {
            status: typeof maybeStatus === "number" ? maybeStatus : typeof responseStatus === "number" ? responseStatus : undefined,
...
export async function analyzeWithGemini(
    ...
    const prompt = buildMountPrompt(text, mode);
...
function buildMountPrompt(text: string, mode: "solo" | "versus"): string {
    const modeLabel = mode === "versus" ? "対戦モード" : "ソロモード";
